...
echo                                                                 %mydate% - %mytime%
echo.
echo  KMS (WinDivert) Activation Windows ^& Office %ver% - mephistooo2 - www.TNCTR.com 
echo.
...
echo ===================================================================================
echo.
Echo.        [1] ACTIVATION (WinDivert Method) START FOR WINDOWS 10 ^& OFFICE
Echo.
Echo.        [2] ACTIVATION TASK SCHEDULER FOR WINDOWS 10 ^& OFFICE
Echo.
Echo.        [3] DELETE ACTIVATION TASK SCHEDULER
Echo.
Echo.        [4] $OEM$ ACTIVATION FOLDER EXTRACT TO DESKTOP
Echo.
Echo.        [5] WINDOWS ^& OFFICE ACTIVATION STATUS CHECK
Echo.
Echo.        [6] DIGITAL ACTIVATION VISIT WEBSITE (TNCTR)
Echo.
Echo.        [7] EXIT 
Echo.
...
:Activate
@echo off
if exist "%SystemRoot%\system32\KMS.dll" goto :KMSInject
set _Debug=0
if %_Debug% EQU 0 (
  set "_Nul_1=1>nul"
  set "_Nul_2=2>nul"
...
set KMS_Port=1688
set RandomLevel=1
set HardwareID=3A1C049600B60076
...
echo.
echo Activation process is performed, please wait...
for %%p in ("%xOS%\KMS.exe") do set "ServerPath=%%~fp"
netsh advfirewall firewall delete rule name="KMS" %_Nul_1_2%
netsh advfirewall firewall add rule name="KMS" dir=in action=allow profile=any program="%ServerPath%" %_Nul_1_2%
cmd /c start /b /high "" "%ServerPath%" -r %RandomLevel% -P %KMS_Port% -H %HardwareID% -R %KMS_RenewalInterval% -A %KMS_ActivationInterval% -T0 -e %_Nul_1_2%

if %winbuild% GEQ 9600 (
route add %KMS_IP% 0.0.0.0 IF 1 %_Nul_1_2%
cmd /c start /b /high "" "%xOS%\KMSClient.exe" %KMS_IP% %_Nul_1_2%
)

for %%A in (14,15,16) do call :officeLoc %%A
call :SPP
call :OSPP
del /f /q "%xOS%\sppchk.txt" %_Nul_1_2%
del /f /q "%xOS%\osppchk.txt" %_Nul_1_2%
del /f /q "%xOS%\key.vbs" %_Nul_1_2%

net stop sppsvc /y %_Nul_1_2% || sc stop sppsvc %_Nul_1_2%
taskkill /t /f /IM KMS.exe %_Nul_1_2%
netsh advfirewall firewall delete rule name="KMS" %_Nul_1_2%
if %winbuild% GEQ 9600 (
taskkill /t /f /IM KMSClient.exe %_Nul_1_2%
route delete %KMS_IP% 0.0.0.0 %_Nul_1_2%
timeout /t 3 /nobreak %_Nul_1_2%
sc stop WinDivert1.4 %_Nul_1_2%
sc delete WinDivert1.4 %_Nul_1_2%
)
...
:sppchk
wmic path %spp% where ID='%app%' get Name >"%xOS%\sppchk.txt"
find /i "Office" %xOS%\sppchk.txt %_Nul_1% && (set office=1) || (set office=0)
find /i "Office 15" %xOS%\sppchk.txt %_Nul_1% && (if %loc_off15% equ 0 exit /b)
find /i "Office 16" %xOS%\sppchk.txt %_Nul_1% && (if %loc_off16% equ 0 exit /b)
...
:osppchk
wmic path %spp% where ID='%app%' get Name >"%xOS%\osppchk.txt"
find /i "Office 14" %xOS%\osppchk.txt %_Nul_1% && (set off14=1&if %loc_off14% equ 0 exit /b) || (set off14=0)
find /i "Office 15" %xOS%\osppchk.txt %_Nul_1% && (if %loc_off15% equ 0 exit /b)
find /i "Office 16" %xOS%\osppchk.txt %_Nul_1% && (if %loc_off16% equ 0 exit /b)
...
echo.
echo ==== ERROR ====
echo KMS Inject Method is installed on the system.
echo Please uninstall KMS Inject Method for use WinDivert Method 
echo.
echo Press any key to continue...
pause >nul
CLS
