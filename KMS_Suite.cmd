@echo off
set ver=v5.7
title KMS ^& Digital ^& Online Activation Suite %ver% by mephistooo2 - TNCTR.com
...
echo                                                                        %mydate% - %mytime%
echo.                                                             
echo  [4;44mKMS ^& Digital ^& Online Activation Suite %ver% - mephistooo2 - www.TNCTR.com[0m
echo. [44m[m
echo  Special thanks: CODYQX4, abbodi1406, qewlpal, s1ave77, cynecx, qad, Mouri_Naruto                                                         
...
echo [37m==========================================================================================[0m
echo.
echo.  [93m[1] ACTIVATION START FOR WINDOWS 10 ^& OFFICE (KMS Inject Method)[0m
echo.
echo.  [91m[2] ACTIVATION START FOR WINDOWS 10 ^& OFFICE (KMS WinDivert Method)[0m
Echo.
Echo.  [94m[3] ACTIVATION START FOR WINDOWS 10 (Digital Activation Method)[0m
echo.
Echo.  [97m[4] ACTIVATION START FOR WINDOWS 10 ^& OFFICE (Online Activation Method)[0m
Echo.
Echo.  [95m[5] WINDOWS ^& OFFICE ACTIVATION STATUS CHECK[0m
echo.
Echo.  [33m[6] OFFICE 2016 / 2019 PRODUCTS CONVERT RETAIL TO VOLUME[0m
echo.
Echo.  [32m[7] KMS ^& DIGITAL ^& ONLINE ACTIVATION VISIT WEBSITE (TNCTR)[0m
Echo.
Echo.  [96m[8] EXIT[0m
Echo.
echo [37m==========================================================================================[1m
...
if errorlevel 7 goto :TNCTR
if errorlevel 6 goto :R2V
if errorlevel 5 goto :Check
if errorlevel 4 goto :Online
if errorlevel 3 goto :Dijital
if errorlevel 2 goto :WinDivert
...
:Dijital
cls
call "%~dp0bin\Digital\Digital.cmd"
:R2V
cls
...
echo.
echo  Special thanks: TNCTR Family - CODYQX4, abbodi1406, qewlpal, s1ave77, cynecx, qad                                                          
echo   SUPPORT MICROSOFT PRUDUCTS:
echo   Windows 7 (VL) - Windows 8 / 8.1 / 10
...
echo ==========================================================================================
echo.
Echo.                 [1] KMS.DIGIBOY.IR         [2] K.ZPALE.COM
ECHO.
Echo.                 [3] M.ZPALE.COM            [4] KMS.SHUAX.COM
Echo.
Echo.                 [5] KMS.LOTRO.CC           [6] KMS.CNLIC.COM
Echo.
Echo.
Echo.   [7] WINDOWS ^& OFFICE ACTIVATION STATUS CHECK 
Echo.
Echo.   [8] EXIT 
Echo.
Echo.   [9] RETURN KMS SUITE MENU
Echo.
echo ==========================================================================================
...
if errorlevel 6 goto :KMS.CNLIC.COM
if errorlevel 5 goto :KMS.LOTRO.CC
if errorlevel 4 goto :KMS.SHUAX.COM
if errorlevel 3 goto :M.ZPALE.COM
if errorlevel 2 goto :K.ZPALE.COM
if errorlevel 1 goto :KMS.DIGIBOY.IR
::===============================================================================================================
:MENU
CLS
mode con cols=90 lines=50
...
:KMS.DIGIBOY.IR
echo.
set KMSIP=kms.digiboy.ir
echo Checking KMS Server...
echo.
ping -n 1 kms.digiboy.ir | findstr TTL && goto Activate
echo.
color 04
echo ==== ERROR ====
echo KMS Server is down or no internet connection...
echo.
...
:K.ZPALE.COM
echo.
set KMSIP=k.zpale.com
echo Checking KMS Server ...
echo.
ping -n 1 k.zpale.com | findstr TTL && goto Activate
echo.
color 04
echo ==== ERROR ====
echo KMS Server is down or no internet connection...
...
:M.ZPALE.COM
echo.
set KMSIP=m.zpale.com
echo Checking KMS Server ...
echo.
ping -n 1 m.zpale.com | findstr TTL && goto Activate
echo.
color 04
echo ==== ERROR ====
echo KMS Server is down or no internet connection...
...
:KMS.SHUAX.COM
echo.
set KMSIP=kms.shuax.com
echo Checking KMS Server ...
echo.
ping -n 1 kms.shuax.com | findstr TTL && goto Activate
echo.
color 04
echo ==== ERROR ====
echo KMS Server is down or no internet connection...
...
:KMS.LOTRO.CC
echo.
set KMSIP=kms.lotro.cc
echo Checking KMS Server ...
echo.
ping -n 1 kms.lotro.cc | findstr TTL && goto Activate
echo.
color 04
echo ==== ERROR ====
echo KMS Server is down or no internet connection...
...
:KMS.CNLIC.COM
echo.
set KMSIP=kms.cnlic.com
echo Checking KMS Server ...
echo.
ping -n 1 kms.cnlic.com | findstr TTL && goto Activate
echo.
color 04
echo ==== ERROR ====
echo KMS Server is down or no internet connection...
...
if %winbuild% LSS 7600 goto :Unsupported
if %winbuild% LSS 9200 set win7=1
if %winbuild% LSS 9600 set KMS_IP=127.0.0.2
if %winbuild% GEQ 9600 reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows NT\CurrentVersion\Software Protection Platform" /v NoGenTicket /t REG_DWORD /d 1 /f %_Nul_1_2%
...
wmic path %spp% where (Description like '%%KMSCLIENT%%') get Name %_Nul_2% | findstr /i Office %_Nul_1% && (set office15=1) || (if not defined win7 echo.&echo %aword% Office 2013/2016/2019 Product Detected...)
...
find /i "Office" %temp%\sppchk.txt %_Nul_1% && (set office=1) || (set office=0)
find /i "Office 15" %temp%\sppchk.txt %_Nul_1% && (if %loc_off15% equ 0 exit /b)
find /i "Office 16" %temp%\sppchk.txt %_Nul_1% && (if %loc_off16% equ 0 exit /b)
if %office% equ 0 wmic path %spp% where ID='%app%' get LicenseStatus | findstr "1" %_Nul_1_2% && (echo.&call :activate %app%&exit /b)
...
find /i "Office 14" %temp%\osppchk.txt %_Nul_1% && (set off14=1&if %loc_off14% equ 0 exit /b) || (set off14=0)
find /i "Office 15" %temp%\osppchk.txt %_Nul_1% && (if %loc_off15% equ 0 exit /b)
find /i "Office 16" %temp%\osppchk.txt %_Nul_1% && (if %loc_off16% equ 0 exit /b)
wmic path %spp% where ID='%app%' get LicenseStatus | findstr "1" %_Nul_1_2% && (echo.&call :activate %app%&exit /b)
wmic path %spp% where (PartialProductKey is not NULL) get ID | findstr /i "%app%" %_Nul_1_2% && (echo.&call :activate %app%&exit /b)
...
wmic path %spp% where (LicenseStatus='1' and GracePeriodRemaining='0' and PartialProductKey is not NULL) get Name %_Nul_2% | findstr /i "Windows" %_Nul_1_2% && (echo "%NameOS% %SP%%xOS%" preactivated.&exit /b)
call :insKey %app%
exit /b
::===============================================================================================================
:offchk
...
if "!ls2!" equ "1" (echo %5 preactivated.&exit /b)
echo.
if "!ls!" equ "1" (echo %3 preactivated.&exit /b)
call :insKey %app%
exit /b
::===============================================================================================================
:offchk15
...
if /i '%app%' equ '9e9bceeb-e736-4f26-88de-763f87dcc485' (
call :offchk "%app%" "19Access2019VL_MAK_AE" "Access 2019 AE"
exit /b
)
call :insKey %app%
exit /b
